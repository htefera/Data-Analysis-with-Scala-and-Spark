...
class WikipediaSuite {
  ...
  @Test def `'makeIndex' creates a simple index with seven entries`: Unit=
  {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
  ...
  @Test def `'occurrencesOfLang' should be case sensitive` : Unit =
              {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    val rdd = sc.parallelize(Seq(WikipediaArticle("title", "Java Jakarta"),
      WikipediaArticle("title2", "A java Jakarta"),
      WikipediaArticle("title2", "A JavaScript Jakarta"),
      WikipediaArticle("title3","Java is a high level programming language" ),
      WikipediaArticle("title4", "Ends with Java")))
       assert((occurrencesOfLang("Java", rdd) == 3))
                  }

  @Test def  `'rankLangs' should not count multiple occurences of the same language in an article` : Unit ={
    ...
    val res = ranked.head._2 == 2 //counts the number of occurrence of Scala in the first article
    val res2=ranked.head._1=="Scala" // check if the head is Scala
    assert(res2)
